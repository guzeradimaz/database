...
begin
    ...
    INTERSECT
    SELECT tables2.table_name FROM all_tables tables2 WHERE OWNER = schema2) 
    LOOP
        SELECT COUNT(*) INTO diff FROM
        (SELECT table1.COLUMN_NAME name,table1.DATA_TYPE FROM all_tab_columns
        table1 WHERE OWNER=schema1 AND TABLE_NAME= same_table.table_name) cols1
        FULL JOIN
        (SELECT table2.COLUMN_NAME name,table2.DATA_TYPE FROM all_tab_columns
        table2 WHERE OWNER=schema2 AND TABLE_NAME = same_table.table_name) cols2
...
drop procedure scheme_tables_order;
