...
CREATE USER pljson IDENTIFIED BY password;

GRANT CREATE SESSION TO pljson;

GRANT UNLIMITED TABLESPACE TO pljson;

ALTER USER pljson QUOTA UNLIMITED ON USERS;
...
CREATE OR REPLACE FUNCTION auto_increment_generator(table_name in varchar2) RETURN varchar2
  AS
  generated_script VARCHAR(1000); BEGIN
  ...
  BEFORE INSERT ON ' || table_name || ' FOR EACH ROW '|| chr(10) || 'BEGIN ' || chr(10) ||
  '	IF inserting THEN ' || chr(10) ||
  '	IF :NEW.ID IS NULL THEN ' || chr(10) ||
  '	SELECT ' || table_name || '_pk_seq' || '.nextval INTO :NEW.ID FROM dual; '|| chr(10) ||
  '		END IF; '|| chr(10) || '	END IF; '|| chr(10) || 'END;';
RETURN generated_script; END auto_increment_generator;
...
CREATE OR REPLACE FUNCTION get_value_from_xml(xml_string IN VARCHAR2, xpath IN VARCHAR2)
    ...
    SELECT EXTRACTVALUE(XMLTYPE(xml_string), xpath ||'[' || i || ']') INTO current_record FROM dual;
    END LOOP;
return xml_property; end get_value_from_xml;
...
CREATE OR REPLACE PACKAGE BODY xml_package AS
  ...
  join_type VARCHAR2(100); join_condition VARCHAR2(100);
  select_query VARCHAR2(1000) :='SELECT'; BEGIN
  ...
  select_query := select_query || ', ' || columns_list(col_index); END LOOP;
  ...
  RETURN select_query; END xml_select;
  ...
  insert_query := insert_query || ', (' || xml_values(i) || ') '; END LOOP;
  ELSE
  SELECT EXTRACT(XMLTYPE(xml_string), 'Operation/Operation').getStringVal() INTO select_query_to_insert FROM dual;
  insert_query := insert_query || ' ' || xml_select(select_query_to_insert); END IF;
  RETURN insert_query; end xml_insert;
  ...
  update_query := update_query || ',' || set_collection(i); END LOOP;
  ...
  table_name VARCHAR2(100); BEGIN
  
  SELECT EXTRACTVALUE(XMLTYPE(xml_string), 'Operation/Table') INTO table_name FROM dual;
  drop_query := drop_query || table_name || ';'; RETURN drop_query;
  END xml_drop;


  FUNCTION xml_create(xml_string IN VARCHAR2) RETURN nvarchar2
  AS
  col_name VARCHAR2(100); col_type VARCHAR(100);
  parent_table VARCHAR2(100); constraint_value VARCHAR2(100);
  ...
  table_constraints XMLRecord := XMLRecord(); i NUMBER := 0;
  BEGIN
  SELECT EXTRACTVALUE(XMLTYPE(xml_string), 'Operation/Table') INTO table_name FROM dual;
  ...
  constraint_value := constraint_value || ' ' || col_constraints(i); END LOOP;
  ...
  IF primary_constraint IS NOT NULL THEN temporal_record :=
  ...
  table_constraints := XMLRecord(); records_length := 0;
  i := 0;
  SELECT EXTRACT(XMLTYPE(xml_string),
  ...
  return create_query; END xml_create;
END xml_package;
