...
FUNCTION xml_create(xml_string IN VARCHAR2) RETURN nvarchar2
    ...
    temporal_string := temporal_string || ', ' || temporal_record(i); 
    END LOOP;
    create_query:= create_query || 'REFERENCES ' || parent_table  || '(' || 'id' || '))';
    END LOOP;
...
DECLARE
    ...
    BEGIN
    query_sql := xml_package.xml_insert( '<Operation>
    <Type>INSERT</Type>
    <Table>students</Table>
    <Columns>
    <Column>id</Column>
...
DECLARE
    query_sql varchar2(1000);
    BEGIN
    query_sql := xml_package.xml_delete( '<Operation>
    <Type>DELETE</Type>
    <Table>students</Table>
    <Where>
    <Conditions>
...
DECLARE
    query_sql varchar2(1000);
    BEGIN
    query_sql := xml_package.xml_update( '<Operation>
    <Type>UPDATE</Type>
    <Table>students</Table>
    <SetOperations>
    <Set>id = 6</Set>
...
DECLARE
    ...
    BEGIN
    sql_query := xml_package.xml_drop( '<Operation>
    <Type>DROP</Type>
    <Table>table1</Table>
    </Operation>');
